...
** Mark Key Theme
The corresponding symbol is =mark=. The default states are normal and visual. See [[#using-only-lispys-mark-commands][here]] for more information. Note that this key theme can be used even if you don't plan to use ~(lispyville-enter-special-when-marking)~ in your configuration.

| key   | command                              |
|-------+--------------------------------------|
| =v=   | ~lispyville-wrap-mark-symbol-visual~ |
| =V=   | ~lispyville-wrap-mark-visual~        |
| =C-v= | ~lispyville-wrap-mark-visual~        |
