...
** C-w Key Theme
The corresponding symbol is =c-w=. The default states are emacs and insert. This is the safe version of ~evil-delete-backward-word~. It will act as ~lispy-delete-backward~ after delimiters (and delete everything within the delimiters).
