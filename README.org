...
** Additional Movement
The corresponding symbol is =additional-movement=. This key theme is the equivalent of cleverparen's additional movement keys. =[= and =]= are like the reverse of ~lispy-flow~. ={= and =}= are like ~lispy-flow~. =(= and =)= are like ~lispy-left~ and ~lispy-right~. See [[More Fluid Transitioning Between Normal State and Special][here]] for some extra information.
